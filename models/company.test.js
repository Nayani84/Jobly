...
describe("findAll", function () {
  ...
  test("works: filter by name", async function () {
    ...
    expect(companies.length).toEqual(1); // Assuming 1 company with 'c1' in the name
  });

  test("works: filter by minEmployees", async function () {
    let companies = await Company.findAll({ minEmployees: 2 });
    expect(companies.length).toEqual(2); // Assuming 2 companies have at least 2 employees
  });

  test("works: filter by maxEmployees", async function () {
    let companies = await Company.findAll({ maxEmployees: 1 });
    expect(companies.length).toEqual(1); // Assuming 1 company has no more than 1 employees
  });
  ...
  test("works: filter by minEmployees and maxEmployees", async function () {
    ...
    expect(companies.length).toEqual(2); // Assuming 2 companies fall within the range
  });
});
